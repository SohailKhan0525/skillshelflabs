...
export default async function Home(){
  ...
  <section className="featured shell"><div className="section-head"><p className="eyebrow">YOUR LIBRARY</p><h2>Start with a skill that has context.</h2></div><div className="skill-grid">{skillList.map(s=><Link href={"/skills/"+s.slug} className="skill" key={s.slug}><span className="tag">{s.category}</span><small>{s.ai_tool}</small><h3>{s.name}</h3><p>{s.description}</p><b>Understand this skill ↗</b></Link>)}</div></section>
